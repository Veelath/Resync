          ...
          <div className="px-4 pt-3.5 pb-2.5 border-b border-gray-100">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              ...
              <button
                ...
                className={`py-1.5 px-2.5 rounded-lg text-xs font-bold transition-all text-center cursor-pointer ${
                  filterType === "strength" ? "bg-emerald-700 text-white" : "bg-emerald-50 text-emerald-700 hover:bg-emerald-100"
                }`}>
                Strengths (3)
              </button>
              <button
                ...
                className={`py-1.5 px-2.5 rounded-lg text-xs font-bold transition-all text-center cursor-pointer ${
                  filterType === "major-issue" ? "bg-rose-700 text-white" : "bg-rose-50 text-rose-700 hover:bg-rose-100"
                }`}>
                Major Issues (3)
              </button>
              <button
                ...
                className={`py-1.5 px-2.5 rounded-lg text-xs font-bold transition-all text-center cursor-pointer ${
                  filterType === "affected-section" ? "bg-blue-700 text-white" : "bg-blue-50 text-blue-700 hover:bg-blue-100"
                }`}>
                Affected (3)
              </button>
            </div>
          ...
          <div className="px-3.5 py-3 flex-1 space-y-1.5">
            {filteredItems.map((item, idx) => (
              <button key={item.id}
                ...
                className={`w-full flex items-start gap-2.5 px-3 py-2.5 rounded-xl text-left border transition-all cursor-pointer ${
                  activeId === item.id ? `${ST[item.type].bg} ${ST[item.type].border} shadow-xs` : "border-transparent hover:bg-gray-50"
                }`}>
...
function LoginScreen({ onNavigate }: { onNavigate: (s: Screen) => void }) {
  ...
  function handleDemoFill() {
    setEmail("maria.santos@dlsu.edu.ph");
    setPw("password123");
  }
  ...
  return (
    <AuthLayout onNavigate={onNavigate}>
      <div>
        ...
        {/* Demo login shortcut helper */}
        <div className="mb-6 p-3 rounded-2xl border border-blue-100 bg-blue-50/60 flex items-center justify-between text-xs">
          <div className="flex items-center gap-2 text-blue-900">
            <span className="text-sm">⚡</span>
            <div>
              <p className="font-semibold text-gray-900 text-[11px]">Evaluator Demo Account</p>
              <p className="text-[10px] text-gray-500 mono">maria.santos@dlsu.edu.ph</p>
            </div>
          </div>
          <button
            type="button"
            onClick={handleDemoFill}
            className="px-2.5 py-1 rounded-lg text-xs font-bold text-white transition-all hover:opacity-90 shadow-sm"
            style={{ background: B }}
          >
            Auto-fill
          </button>
        ...
        <button
          type="button"
          onClick={() => { setLoading(true); setTimeout(() => { setLoading(false); onNavigate("dashboard"); }, 900); }}
