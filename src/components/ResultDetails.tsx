          ...
          <div className="hidden md:flex items-center bg-slate-100 p-0.5 rounded-lg border border-slate-200 mr-2">
            <button
              ...
              className={`px-2.5 py-1 text-xs font-bold rounded-md transition-all cursor-pointer ${
                viewMode === 'manuscript' ? 'bg-white text-indigo-600 shadow-2xs' : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              Figma View
            </button>
            <button
              ...
              className={`px-2.5 py-1 text-xs font-bold rounded-md transition-all cursor-pointer ${
                viewMode === 'tabs' ? 'bg-white text-indigo-600 shadow-2xs' : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              Detailed Tabs
            </button>
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 rounded-xl border flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-colors ${
                          !badge.isLive ? 'bg-rose-50/40 border-rose-200' : 'bg-slate-50/80 border-slate-200/80'
                        }`}
                      >
                          ...
                          <div className="space-y-1 min-w-0">
                            ...
                            {link && (
                              <a
                                ...
                                className={`text-[11px] font-mono hover:underline truncate block max-w-md ${
                                  !badge.isLive ? 'text-rose-600' : 'text-slate-400'
                                }`}
                              >
                                {link}
                              </a>
              ...
              <div className="space-y-4 animate-fade-in">
                <div className="flex items-center justify-between">
                  <span className={`text-[10px] font-bold uppercase px-2.5 py-0.5 rounded-full ${
                    (selectedIssue.inconsistencyType || '').toLowerCase().includes('logic')
          ...
          <div className="flex border-b border-slate-200">
            {tabs.map(tab => (
              <button
                ...
                className={`flex items-center gap-2 px-6 py-4 text-sm font-bold border-b-2 -mb-[2px] transition-all cursor-pointer ${
