...
export default function ScanForm({
  ...
  return (
    <div className="w-full max-w-5xl mx-auto py-2 animate-fade-in text-left">
      ...
      {/* Right: Quick Start Guide stepper */}
      <div className="lg:col-span-5 xl:col-span-4 hidden lg:block">
        <div className="sticky top-24 rounded-2xl border border-slate-200 bg-slate-50 p-6 shadow-sm break-inside-avoid">
          {/* header */}
          <div className="flex items-center gap-2.5 mb-6">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 bg-indigo-50">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3.5 h-3.5 text-indigo-700"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg>
            </div>
            <h2 className="text-base font-bold text-slate-900">Before you scan</h2>
          </div>

          {/* steps */}
          <div className="relative">
            {/* vertical connector line */}
            <div className="absolute left-[15px] top-8 bottom-8 w-px bg-slate-200 z-0"/>

            <div className="space-y-7 relative z-10">
              {[
                {
                  n: "1",
                  title: "Prep your document",
                  body: "Ensure your chapter headings (e.g., 'Chapter 1: Introduction') are clearly labeled so the engine can parse them correctly.",
                },
                {
                  n: "2",
                  title: "Check file limits",
                  body: "Upload a .docx file under 25 MB, or ensure your Google Docs link is set to 'Anyone with the link can view'.",
                },
                {
                  n: "3",
                  title: "Attach a template",
                  body: "If your university uses a specific format, upload it below to improve AI chapter mapping accuracy.",
                  optional: true,
                },
              ].map(({ n, title, body, optional }) => (
                <div key={n} className="flex gap-4">
                  {/* number badge */}
                  <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-white text-xs font-bold z-10 bg-indigo-700 shadow-sm">
                    {n}
                  </div>
                  <div className="pt-0.5 pb-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {optional && (
                        <span className="text-[10px] font-semibold text-slate-400 bg-slate-200 px-1.5 py-0.5 rounded-full">Optional</span>
                      )}
                    </div>
